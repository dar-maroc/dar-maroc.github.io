...
-- Section "Partenaires" : logos défilants de la page d'accueil
...
create policy "contenu_public_select" on public.partners for select using (actif = true);
...
create policy "contenu_phase1_write" on public.partners for all using (true) with check (true);
...
insert into public.partners (nom_fr, nom_ar, icon, ordre) values
  ('Marjane', 'مرجان', 'fa-basket-shopping', 0),
  ('CTM', 'CTM', 'fa-bus', 1),
  ('Maroc Telecom', 'اتصالات المغرب', 'fa-tower-cell', 2),
  ('Atlas Copco', 'أطلس كوبكو', 'fa-industry', 3),
  ('Ciments du Maroc', 'إسمنت المغرب', 'fa-truck-ramp-box', 4),
  ('Alliance Assurances', 'أليانس للتأمين', 'fa-shield-halved', 5),
  ('Attijariwafa Bank', 'التجاري وفا بنك', 'fa-landmark', 6),
  ('Royal Air Maroc', 'الخطوط الملكية المغربية', 'fa-plane', 7)
